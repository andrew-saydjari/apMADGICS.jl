...
@everywhere begin
    function pipeline_single_spectra(argtup; caching=true, sky_caching=false, cache_dir="../local_cache", inject_cache_dir=prior_dir2*"2023_07_11/inject_local_cache")
        ival = argtup[1]
        intup = argtup[2:end]
        out = []
    ...
    function multi_spectra_batch(indsubset; out_dir="../outdir", ddstaronly=false)
        ...
        if !isfile(savename)
            ...
            if prior_load_needed
                ...
                f = h5open(prior_dir2*"2023_07_12/sky_priors/APOGEE_skycont_svd_30_f"*lpad(adjfibindx,3,"0")*".h5")
                global V_skycont = read(f["Vmat"])
                chebmsk_exp = convert.(Bool,read(f["chebmsk_exp"]))
                close(f)

                f = h5open(prior_dir2*"2023_07_12/sky_priors/APOGEE_skyline_svd_120_f"*lpad(adjfibindx,3,"0")*".h5")
...
# for adjfibindx=345:345
    subiter = deserialize(prior_dir2*"2023_07_11/injection_input_lst_"*lpad(adjfibindx,3,"0")*".jdat")
