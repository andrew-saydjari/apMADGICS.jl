    ...
    function multi_spectra_batch(indsubset; out_dir="../outdir", ddstaronly=true)
        ...
        if !isfile(savename)
            ...
            if prior_load_needed
                ...
                f = h5open(prior_dir*"2024_02_03/sky_priors/APOGEE_skyline_bright_svd_60_f"*lpad(adjfibindx,3,"0")*".h5") #revert temp
                global V_skyline_bright = 100*read(f["Vmat"])
                submsk_bright = convert.(Bool,read(f["submsk"]))
                close(f)

                f = h5open(prior_dir*"2024_02_03/sky_priors/APOGEE_skyline_faint_svd_60_f"*lpad(adjfibindx,3,"0")*".h5") #revert temp
                global V_skyline_faint = 100*read(f["Vmat"])
