...
@everywhere begin
    function pipeline_single_spectra(argtup; caching=true, sky_caching=true, sky_off=false, cache_dir="../local_cache", inject_cache_dir=prior_dir2*"2023_08_22/inject_local_cache_2")
    ...
    function multi_spectra_batch(indsubset; out_dir="../outdir", ddstaronly=true)
        ...
        if !isfile(savename)
            ...
            if prior_load_needed
                ...
                f = h5open(prior_dir2*"2023_09_09/star_priors/APOGEE_starCor_svd_50_subpix_f"*lpad(adjfibindx,3,"0")*".h5")
...
for adjfibindx = 295:295 #1:600 #295, 245
    subiter = deserialize(prior_dir2*"2023_09_10/injection_input_lst_"*lpad(adjfibindx,3,"0")*".jdat")
